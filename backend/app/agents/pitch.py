...
async def run_pitch_agent(
    ...
    system_instruction = (
        "You are an elite Startup Pitch Coach and Venture Capital (VC) partner. Your task is to write the detailed contents of "
        "a 10-slide investor pitch deck based on the startup's generated business details.\n\n"
        "Ensure the content is highly professional, localized, and quantitative. The slide structure should follow standard pitch templates, avoiding default tech-only metrics for non-tech businesses:\n"
        "1. Title (Brand & Slogan)\n"
        "2. The Problem (The target customer's core pain points, market inefficiencies, or operational bottlenecks)\n"
        "3. The Solution (Your core unique value proposition, product delivery edge, or structural advantage)\n"
        "4. Product Features & MVP (Key MVP features, core user flow, and the delivery technology stack)\n"
        "5. Market Size (TAM, SAM, SOM with bottom-up calculations reflecting regional purchasing power parity)\n"
        "6. Business Model (Monetization model, localized pricing tiers, and estimated margins/unit economics)\n"
        "7. Competition (Competitor landscape comparison, competitor weaknesses, and your startup's SWOT-derived moats)\n"
        "8. Go-To-Market (Acquisition channels, marketing timelines, and concrete CAC/conversion metrics localized to the location)\n"
        "9. Financial Projections (3-Year revenue, expense, and profit growth projections matching the SOM calculations)\n"
        "10. The Ask & Next Steps (Funding target aligned with the budget tier, detailing how it will be spent: marketing, equipment/inventory, hiring, or operations)\n\n"
    ...
    try:
        ...
        fallback_prompt = ChatPromptTemplate.from_messages([
            ...
            ("system", "You are a pitch coach. Respond ONLY with valid JSON (no markdown) that matches: "
             "{{\"slides\": [{{\"slide_number\": <number>, \"title\": <string>, \"bullet_points\": [<strings>], "
             "\"design_note\": <string>}}]}}"),
